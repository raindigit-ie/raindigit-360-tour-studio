...
function testContract() {
  ...
  const disabled = tourMonitoringConfig({ identity, slug: "test-tour", environment: {} });
  assert(disabled.enabled === false && disabled.dsn === "", "A build without a DSN must stay inert.");
  ...
  expectThrow(
    () => tourMonitoringConfig({
      identity,
      slug: "test-tour",
      environment: { RAINDIGIT_TOUR_SENTRY_DSN: "https://key@o1.ingest.sentry.io/123" },
    }),
  ...
  assert(!injected.includes("</script><script>alert"), "Monitoring config can break out of its script element.");
}

async function testBrowserRuntime() {
  const monitor = await readFile(join(projectRoot, "web-tour", "js", "generated", "tour-monitoring.min.js"), "utf8");
  ...
  try {
    ...
    await context.route("**/*", async (route) => {
      ...
      if (url.pathname === "/js/tour-monitoring.js") {
        await route.fulfill({ status: 200, contentType: "application/javascript", body: monitor });
    ...
    await productionPage.goto("https://cdn.raindigit.ie/tours/test-tour/multires-deadbeef0000/index.html?signed=secret#private");
    await productionPage.waitForFunction(() => Boolean(window.__rainDigitTourMonitoring));
    assert(sdkRequests === 0, "The Sentry SDK was requested during a healthy production boot.");
    ...
    await productionPage.waitForFunction(() => window.__rainDigitCaptured?.length === 1);
    assert(sdkRequests === 1, "The Sentry SDK was not loaded exactly once after a terminal failure.");
    const evidence = await productionPage.evaluate(() => {
      ...
      const sanitized = options.beforeSend({
        ...
        contexts: { tour: { token: "secret", phone: "+353 87 123 4567", sceneId: "scene-003" } },
        exception: { values: [{ value: "Bad https://example.com/a?token=secret", stacktrace: { frames: [{ filename: "https://cdn.raindigit.ie/app.js?token=secret", vars: { token: "secret" } }] } }] },
      });
      ...
      return { options, sanitized, duplicate, captured: window.__rainDigitCaptured };
    });
    assert(evidence.options.defaultIntegrations === false, "Default Sentry browser integrations were enabled.");
    assert(evidence.options.tracesSampleRate === 0 && evidence.options.sendDefaultPii === false, "Privacy or quota controls are incomplete.");
    assert(evidence.sanitized.user == null && evidence.sanitized.breadcrumbs == null, "PII-bearing Sentry fields were retained.");
    assert(!JSON.stringify(evidence.sanitized).includes("secret"), "A URL query, cookie, header or frame variable survived redaction.");
    assert(evidence.duplicate === false && evidence.captured.length === 1, "Duplicate terminal failures were not collapsed.");

    const devPage = await context.newPage();
    await devPage.goto("https://pub-example.r2.dev/tours/test-tour/multires-deadbeef0000/index.html");
    await devPage.waitForFunction(() => Boolean(window.__rainDigitTourMonitoring));
    ...
    assert(devAccepted === false && sdkRequests === 1, "A DEV/R2 origin consumed production monitoring.");
    await context.close();
  } finally {
    await browser.close();
  }
...
async function testRuntimeRevisionWiring() {
  const source = await readFile(join(projectRoot, "scripts", "revise-multires-runtime.mjs"), "utf8");
  assert(
    source.includes("environment: productionTourMonitoringEnvironment()"),
...
console.log("Production-only tour monitoring passed: exact-origin gate, lazy SDK, tile suppression, redaction and dedupe.");
