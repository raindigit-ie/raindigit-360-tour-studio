#!/usr/bin/env python3
"""Benchmark IAT exposure/enhancement on real X4 panorama crops."""
...
OUT = LAB / "iat-output"
REPORT = LAB / "iat-report"
IAT_ROOT = LAB / "tools" / "illumination-adaptive-transformer" / "IAT_enhance"
MODEL_ROOT = IAT_ROOT / "model"
...
MODELS = [
    {"label": "iat-exposure", "path": IAT_ROOT / "best_Epoch_exposure.pth"},
    {"label": "iat-enhance-lol-v1", "path": IAT_ROOT / "best_Epoch_lol_v1.pth"},
]
...
def load_iat(model_path: Path, device: torch.device):
    if str(IAT_ROOT) not in sys.path:
...
def run_iat(model, image: Image.Image, device: torch.device) -> Image.Image:
    array = np.asarray(image.convert("RGB"), dtype=np.float32) / 255.0
    tensor = torch.from_numpy(array).permute(2, 0, 1).unsqueeze(0).to(device)
    with torch.no_grad():
        _, _, enhanced = model(tensor)
    enhanced = enhanced.clamp(0.0, 1.0).squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
    return Image.fromarray(np.asarray(enhanced * 255.0, dtype=np.uint8), "RGB")
...
def main() -> None:
    ...
    if missing:
        raise SystemExit("Missing IAT model(s): " + ", ".join(missing))

    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    loaded_models = []
    for model in MODELS:
        started = time.time()
        loaded_models.append((model, load_iat(Path(model["path"]), device), round(time.time() - started, 3)))

    rows = []
    sample_results = []
    labels = ["reference", "iat-exposure", "exposure-blend35", "iat-enhance-lol-v1", "enhance-blend35"]
    for sample in SAMPLES:
        ...
        for model, net, load_seconds in loaded_models:
            ...
            started = time.time()
            enhanced = run_iat(net, reference, device)
            ...
            blend_label = "exposure-blend35" if label == "iat-exposure" else "enhance-blend35"
            blended = blend(reference, enhanced, 0.35)
    ...
    contact = REPORT / "iat-enhance-contact-sheet.jpg"
    make_contact_sheet(rows, labels, contact)
    report = {
        "createdAt": time.strftime("%Y-%m-%dT%H:%M:%S%z"),
        "method": "IAT exposure/enhance pretrained models on real X4 crops, direct output plus 35% blend with the camera JPG.",
        "device": str(device),
        "models": [
            {
                "label": str(model["label"]),
    ...
    report_path = REPORT / "iat-enhance-benchmark-report.json"
