...
async function main() {
  ...
  try {
    ...
    const serverBuild = await requestJson(`${baseUrl}/__tour-editor/build-release?workspace=1`, { method: "POST" });
    assert(serverBuild.prunedScenes.some((scene) => scene.id === "scene-099"), `Server build did not report pruning the unused scene: ${JSON.stringify(serverBuild)}`);
    const prunedProject = JSON.parse(await readFile(workspaceProjectPath, "utf8"));
    const prunedDraft = JSON.parse(await readFile(workspaceDraftPath, "utf8"));
    assert(!prunedProject.scenes.some((scene) => scene.id === "scene-099") && !prunedProject.rooms.some((room) => room.id === "room-unused"), "Server build did not remove the unused scene and empty room from the workspace.");
    assert(!prunedDraft.addedHotspots?.["scene-099"] && !prunedDraft.sceneMetadata?.["scene-099"], "Server build did not remove the unused scene from the draft.");
